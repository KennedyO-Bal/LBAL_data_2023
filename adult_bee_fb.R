...
#load count files----
Counts<-read.delim("adult.fatbody_feature.counts new.txt", header = T, row.names = 1, stringsAsFactors = F, check.names=FALSE)
colSums(Counts)
str(Counts)
...
sample.info <- sample.info.all[which(sample.info.all$sex == 'female' & 
                                       ...
                                       sample.info.all$tissue == 'fatbody' &
...
#save(dds, file = "filtered_normalized.dds_adult.brain.RData")
...
# plot latent variables by probe
...
## To be used for comparing Workers to all reprods(Queens and sol_reproductive)
...
#PCA, Heatmatrix with SVA variables included.
...
pcaData<-plotPCA(vsd.sva, intgroup=c( "behavior"),returnData = T)##including returnData=T returns the data frame useful for plotting in ggplot
...
fat_body_anov<-aov(pcaData$PC1 ~ behavior,
                   data=pcaData)
summary(fat_body_anov)
...
#write.csv(dds_vst_transposed.sva, file = "vst_counts_core_for WGCNA_adult.fatbodies_sva.csv")
...
length(Q_W_results.sva_sig$padj)#376
...
length(W_S_results.sva_sig$padj)#200
...
length(Q_F_results.sva_sig$padj)#42
...
F_S_results.sva_sig <- as.data.frame(F_S_results.sva_sig)
length(F_S_results.sva_sig$padj)#0
...
length(F_W_results.sva_sig$padj)#601

## saving the DEG files for each comparison

setwd()

#write.csv(Q_W_results.sva_sig, file = "Q_W_DEGs_adult_fb.csv")
#write.csv(Q_S_results.sva_sig, file = "Q_S_DEGs_adult_fb.csv")
#write.csv(W_S_results.sva_sig, file = "W_S_DEGs_adult_fb.csv")
#write.csv(Q_F_results.sva_sig, file = "Q_F_DEGs_adult_fb.csv")
#write.csv(F_S_results.sva_sig, file = "F_S_DEGs_adult_fb.csv")
#write.csv(F_W_results.sva_sig, file = "F_W_DEGs_adult_fb.csv")
...
W_vs_QS_results_sig <- subset(W_vs_QS_results.sva, padj < 0.001)
W_vs_QS_results_sig <- as.data.frame(W_vs_QS_results_sig)
length(W_vs_QS_results_sig$padj)#266
#write.csv(W_vs_QS_results_sig, file = "W_vs_QS_FB_results_sig.csv")
...
EnhancedVolcano(W_vs_QS,
                ...
                title = 'Volcano Plot',
                subtitle = 'Workers vs Reproductive_fb',
                pCutoff = 0.001,
                FCcutoff = 0,
                pointSize = 3.0,
                labSize = 3.0,
...
length(Q_F.Genes)
length(F_W.Genes)
length(F_S.Genes) # 0 DEGs

#convert these from a list of character vectors to a presence/absence matrix

UpSetR::upset(fromList(list(QvW = Q_W.Genes,  WvS = W_S.Genes, QvF = Q_F.Genes, FvW = F_W.Genes, QvS = Q_S.Genes)),
              nsets = 5,
              point.size = 4, 
