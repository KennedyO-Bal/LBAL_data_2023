...
Counts<-read.delim("sunny&shady_fatbody new.txt", header = T, row.names = 1, stringsAsFactors = F, check.names=FALSE)
colSums(Counts)
str(Counts)
...
vst.df <- t(vst.df)
#write.csv(vst.df, file = "counts_core_for WGCNA_pupa.sunny v shady fb_sva.csv")
...
fit <- lm(-pcaData1$PC1 ~ vsd$Location, data = pcaData1)
...
#write.csv(dds_vst.df, file = "counts_core_for WGCNA_pupa.fb_brood 1.csv")
...
length(SunnyVshady_results_sig$padj)#2028
...
#write.csv(SunnyVshady_results_sig, file = "SunnyVShady_pupa_fb_brood 1.csv")
...
EnhancedVolcano(SunnyVshady_results,
                lab = lab_italics,
                xlim = c(-16, 16), ylim = c(0, 60),
                x = 'log2FoldChange',
                y = 'pvalue',
                selectLab = NA,
                xlab = bquote(~Log[2]~ 'fold change'),
                title = 'Volcano Plot',
                subtitle = 'social biased v sol-biased_fb',
                pCutoff = 0.001,
                FCcutoff = 0,
                pointSize = 3.0,
                labSize = 3.0,
